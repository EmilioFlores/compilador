...
dirProcedimientos :: int buscaDireccion(string id){
    int iVar;
...
dirProcedimientos :: int buscaTipo(string tipo){
...
dirProcedimientos :: int buscaVariable (string id){
    int val=-1;
...
dirProcedimientos :: int checaTipo(string id){
...
dirProcedimientos :: bool crearVariable(string tipo, string id, bool privacidad,int direccion){
    variable v;
...
dirProcedimientos :: void creaMemoriaObjeto(int bloque){//crea la memoria para un objeto con la informacion en el bloque que lo define
    Objeto obj;
...
dirProcedimientos :: bool crearObjeto(string tipo){
    for (int i = 0 ; i < tipos.size() ; i++){
        if (tipos[i]==tipo){
...
dirProcedimientos :: int espacioMemoriaObjeto(int tipo){
    switch (tipo) {
        case 0:
            memoria.cantBanObj++;
...
dirProcedimientos :: bool agregaParametro (string tipo, string id, int direccion){
...
dirProcedimientos :: bool checaArgumentoID(string id){
 ...
 dirProcedimientos :: int checaEstructura(string id){
    int iV = buscaVariable(id);
    if (iV==-1)
        return -1;
...
dirProcedimientos :: bool checaArgumentoTipo(int tipo){
    int iAux = ultVariabe;// se guardan para mantener los datos del metodo que llamo los argumentos
    string sAux = nombreUltVar;
    if (vVariables[ultVariabe].estructura!=1){
        cout << "La variable "<<nombreUltVar<<" no es un metodo no debe tener argumentos"<<endl;
...
dirProcedimientos :: bool checaCompletezPred(bool metodo){
...
dirProcedimientos :: bool checaPredicado(string id){
...
dirProcedimientos :: bool crearMetodo (string tipo, string id, int direccion){
    variable v;
    v.tipo = buscaTipo(tipo);
    if (v.tipo==-1){
        cout << tipo << " "<< id << " Tipo Inexistente"<<endl;
...
dirProcedimientos :: void copiaDirecciones(int tipo,int espacio, int val){
    switch (tipo) {
        case 0:
...
dirProcedimientos :: void subsDireccionesObj(int dir){
...
dirProcedimientos :: void terminaBloque(){
...
dirProcedimientos :: void inicializaMemoria(){
